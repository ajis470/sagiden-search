...
// コメント（published のみ）
$stmt = $pdo->prepare('SELECT id, body, source, created_at FROM sagiden_comments WHERE phone_number_id = ? AND status = "published" ORDER BY created_at DESC');
$stmt->execute([$phone_id]);
$sagiden_comments = $stmt->fetchAll(PDO::FETCH_ASSOC);

// AI要約
$stmt = $pdo->prepare('SELECT summary, recommended_action, danger_rank, generated_at FROM sagiden_ai_summaries WHERE phone_number_id = ?');
$stmt->execute([$phone_id]);
