...
以下のJSON形式のみで回答してください。余分なテキストは不要です：
{
  ...
  "recommended_action": "発信元が詐欺・フィッシングとほぼ判断できる場合は着信拒否を推奨してよい。正規の企業・機関と判断できる場合は着信拒否を推奨しない（なぜなら受信者に非がある場合が想定されるので）。判断が難しい場合は「悪質と感じた場合は着信拒否も選択肢の一つです」と添える。詐欺・悪質業者が疑われるケースでは消費者センターへの相談を案内することも検討してよい。この電話を受けた人への有益な案内を1〜2行で書く。",
  "danger_rank": "C/B/A/S/SS/SSSのいずれか1つ",
  "highlights": [
    "口コミの中で特に参考になる情報を、口コミの文章をそのまま使わず、内容を参考にして第三者視点で書き直したもの（1〜5件）"
  ]
}`;
